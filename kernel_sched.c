...
TCB* spawn_thread(PCB* pcb, void (*func)())
{
  ...
  tcb->counterMutex = 0;     /**< This is a counter for How many time by time the mutex Cause was called for the current thread */
  tcb->priority = 4;
...
static int QuantumCounter=0;/* our current yields called , grows after every yield call*/
/* Core control blocks */
CCB cctx[MAX_CORES];
...
rlnode SCHED_QUE[LEVELS];     /*The array of linked round robin lists*/
...
static void sched_queue_add(TCB* tcb)
{
  ...
  rlist_push_back(& SCHED_QUE[tcb->priority], & tcb->sched_node);
...
static TCB* sched_queue_select()
{
 ...
 // TimerDuration curtime = bios_clock();
  ...
  /* Get the head of the SCHED_QUE list array */
  /*Looking for the first head of list (NON NULL) so it will be our selected queue */
  rlnode * sel;
  do{
    priority--;
    sel = rlist_pop_front(& SCHED_QUE[priority]);
  }
...
void PrioritizeThreads(enum SCHED_CAUSE cause, TCB* tcb)
{
  ...
  else if(cause == SCHED_IO && tcb->priority < 4){
    (tcb->priority)++;
  }else{;}
}
...
/*  @ PriorityBoosting
 *
 *   This function works periodically (MaxQuantums) in order to
 *   re-surface thread of very low priority and run them. We look for the head of the
 *   the head of the rlist in the SCHED_QUE (or priority Table) with the lowest priority
 *   and we push it to the highest priority list                           
 */  
void PriorityBoosting(rlnode* priorityTable)
{
  int i = 0; 
  QuantumCounter = 0; /* Initialising the Quantum Counter for the next round */
  rlnode *current;     /*temporary rlnode */
  current = NULL; 
  do
  {
    current = rlist_pop_front(&(priorityTable[i])); /* looking for NON NULL rlist head */
    i++;
  } while(i<LEVELS && current->tcb == NULL);
  if(current->tcb != NULL && current->tcb->state == READY) /* we need a READY state thread to avoid problems */
  {
    current->tcb->priority = 4; /* re-prioritise the thread */
...
{ 
  ...
  /* Altering the thread's priority*/
    PrioritizeThreads(cause,current);

  /* Increamenting the Quantum Counter */
    QuantumCounter++;
    if(QuantumCounter>MaxQuantums)
      PriorityBoosting(SCHED_QUE);
