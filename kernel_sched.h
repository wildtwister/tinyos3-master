...
typedef struct thread_control_block
{
  ...
  int priority;          /*The priority of the thread in the scheduler(0-4) 4 is the high priority*/
  int counterMutex;     /**< This is a counter for How many time by time the mutex Cause was called for the current thread */
...
/*Gives the current thread its priority based on Yield cause (Quantum time or I0)*/
void PrioritizeThreads(enum SCHED_CAUSE cause, TCB* tcb);

void PriorityBoosting(rlnode* priorityTable);


#define LEVELS (5) /*QUEUE LEVELS*/
/**
...
/* Maximum threads yielded before we move the lower priority threads in higher priority */
#define MaxQuantums (5)
/* A maximum counter for the times a thread will cause Mutec Yield*/
